...
export class LoanPaymentComponent implements OnInit {
  ...
  trackId(_index: number, item: ILoanPayment): number {
    return item.id!;
  }

  delete(loanPayment: ILoanPayment): void {
    const modalRef = this.modalService.open(LoanPaymentDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ILoanPayment[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
