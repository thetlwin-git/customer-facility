...
export class CreditFacilityComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.creditFacilityService
      ...
      .subscribe({
        next: (res: HttpResponse<ICreditFacility[]>) => {
  ...
  trackId(_index: number, item: ICreditFacility): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ICreditFacility[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/credit-facility'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
