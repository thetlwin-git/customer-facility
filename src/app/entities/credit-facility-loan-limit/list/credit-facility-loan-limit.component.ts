...
export class CreditFacilityLoanLimitComponent implements OnInit {
  creditFacilityLoanLimits?: ICreditFacilityLoanLimit[];
  isLoading = false;
  totalItems = 0;
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.creditFacilityLoanLimitService
      ...
      .subscribe({
        next: (res: HttpResponse<ICreditFacilityLoanLimit[]>) => {
  ...
  trackId(_index: number, item: ICreditFacilityLoanLimit): number {
    return item.id!;
  }

  delete(creditFacilityLoanLimit: ICreditFacilityLoanLimit): void {
    const modalRef = this.modalService.open(CreditFacilityLoanLimitDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.creditFacilityLoanLimit = creditFacilityLoanLimit;
  ...
  protected onSuccess(data: ICreditFacilityLoanLimit[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/credit-facility-loan-limit'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.creditFacilityLoanLimits = data ?? [];
